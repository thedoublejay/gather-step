...
Use Gather Step tools before reasoning about cross-repo impact, tracing a request path, or planning a refactor. Gather Step surfaces what grep cannot: edges between services, contract ownership, and downstream breakage from type changes.
...
**When it returned an incorrect path or symbol:** Note it:
