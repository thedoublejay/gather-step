...
- **`pr-review` AI-contract deltas** — the `DeltaReport` gains an `ai_contracts` section reporting added / removed / changed structured-output contracts (keyed by call site) by `source_type_name`, `inference_kind`, and `schema_format`, parallel to `payload_contracts`. Schema-field-level diffs and `provider` / `model` / `temperature` facets are scaffolded in the report structure but not yet populated by the producer; those signals will light up in a later release. The removed-surface pass also flags removed AI surfaces with surviving consumers.
